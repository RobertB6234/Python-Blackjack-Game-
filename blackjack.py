...
Establishes a player class and dealer class and game mechanics for a blackjack card game
'''
...
class Dealer(Player):
	...
	def hit(self,deck):
		'''Add cards while point < 17, then allow all to be shown.'''
...
class Blackjack(object):
	...
	def getPlayerCards(self):
		'''Returns a list of the players cards'''
		return self._player.getCards()
		
	def getDealerCards(self):
		'''Returns a list of the dealers cards'''
		return self._dealer.getCards()
		
	def hitPlayer(self):
		'''Deals a card to the player. Returns a tuple of the card and the players  points'''
	...
	def hitDealer(self):
		'''Deals card to the dealer until an outcome occurs. Returns a string representing the outcome.'''
